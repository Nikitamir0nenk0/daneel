-- showcase_v7.sql
-- moscow_shops и shops_info объединены в один CTE shops
-- retail_shops_dict больше не используется
...
sales AS (
    ...
    WHERE r.operation_day >= today() - 20
        ...
        AND r.shop IN (SELECT shop_id FROM shops)
...
stock AS (
    ...
    WHERE ondate >= toDateTime(today() - 21)
        AND ondate <  toDateTime(today())
...
final AS (
    SELECT
        ...
        additional_images_cnt
    FROM sales
    INNER JOIN shops        ON sales.shop_id  = shops.shop_id
